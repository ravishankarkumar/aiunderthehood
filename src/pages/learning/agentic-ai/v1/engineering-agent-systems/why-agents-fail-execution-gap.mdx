...
This article builds directly on the foundational agent loop from [What is an AI Agent?](/learning/agentic-ai/v1/foundations/core-concepts/what-is-an-agent).

It also connects to [The Memory Hierarchy of Agents](/learning/agentic-ai/v1/foundations/memory-systems-rag/memory-hierarchy), because state drift is often a memory architecture problem before it is a prompting problem.

For runtime structure, it connects to [Designing a Simple Agent State Machine](/learning/agentic-ai/v1/foundations/agent-internals/state-machine), where loops, transitions, and termination conditions become implementation concepts.
...
Continue to [From DAGs to State Machines](/learning/agentic-ai/v1/engineering-agent-systems/from-dags-to-state-machines), where we turn these failure modes into a better abstraction for loops, transitions, retries, and runtime state.

...
A mature evaluation pipeline acts as the **continuous integration system for agents**: Agent CI.
...
6. **Replay Harness** — Re-running saved trajectories against new prompts, models, tools, and policies.
...
- Did tool arguments change in a risky way?
- Did the agent ignore an approval gate or permission boundary?
...
Agent CI treats every prompt, model, tool, memory, and policy change like a software change that can regress.

A practical Agent CI suite includes:

- **Golden trajectories**: known-good runs with expected tool calls, observations, and final outcomes.
- **Trace diffing**: comparisons of plans, tool calls, retries, refusals, and final answers.
- **Prompt-injection regression tests**: adversarial documents, tool outputs, and memory records that try to override instructions.
- **Cost and latency budgets**: maximum token use, step count, tool calls, wall-clock time, and model spend.
- **Safety gates**: automatic failures for unauthorized tools, missing approvals, unsafe filesystem access, or policy violations.
- **Replay tests**: deterministic replays where tool outputs are fixed so behavior changes are easier to isolate.

The goal is not to make agents perfectly deterministic. The goal is to notice when their behavior changes in ways that matter.
...
- Keep adversarial and safety regression suites separate from ordinary quality tests so failures are easier to triage.
- Store raw traces for failed runs so developers can replay the exact execution path.
...
→ Continue to **[10.1 — The Small Model Strategy](/learning/agentic-ai/v1/foundations/high-perf-engineering/small-model)**

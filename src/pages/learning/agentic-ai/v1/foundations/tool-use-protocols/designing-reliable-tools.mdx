...
Similar patterns apply across agent runtimes and frameworks.
...
> **Systems Insight**
>
> Tools are not just functions. They are contracts between the agent and the external world. The schema, permission model, retry behavior, and output envelope all shape whether model intent becomes safe execution.
>
> This is why tool design belongs inside the control architecture of an agent system, not just the integration layer. See [Controlled Agency](/learning/agentic-ai/v1/engineering-agent-systems/controlled-agency-tools-safety) for the systems-level view.
...
→ Continue to **[4.3 — The Model Context Protocol (MCP)](/learning/agentic-ai/v1/foundations/tool-use-protocols/mcp)**
